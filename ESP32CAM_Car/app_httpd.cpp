...
String getParamValue(const char* uri, const char* paramName) {
  String uriString = String(uri);
  String paramString = String(paramName) + "=";
  
  // Find the parameter in the URI string
  int paramStartIndex = uriString.indexOf(paramString);
  if (paramStartIndex == -1) {
    return "";  // Parameter not found
  }

  // Extract the value after the parameter
  paramStartIndex += paramString.length();
  int paramEndIndex = uriString.indexOf('&', paramStartIndex);  // Find next '&' or end of string
  if (paramEndIndex == -1) {
    paramEndIndex = uriString.length();  // No '&' found, take the rest of the string
  }

  // Return the extracted value
  return uriString.substring(paramStartIndex, paramEndIndex);
}
...
static esp_err_t step_go_handler(httpd_req_t *req){
    WheelAct(LOW, HIGH, LOW, HIGH);
    delay(100);
    WheelAct(LOW, LOW, LOW, LOW);
    Serial.println("Step Go");
    httpd_resp_set_type(req, "text/html");
    return httpd_resp_send(req, "OK", 2);
}
...
static esp_err_t command_handler(httpd_req_t *req){
    String direction = getParamValue(req->uri,"direction");
    int step = getParamValue(req->uri,"step").toInt();
...
static esp_err_t bling_handler(httpd_req_t *req){
    digitalWrite(gpLed, HIGH);
    delay(100);
    digitalWrite(gpLed, LOW);
...
static esp_err_t step_left_handler(httpd_req_t *req){
    WheelAct(HIGH, LOW, LOW, HIGH);
    delay(100);
    WheelAct(LOW, LOW, LOW, LOW);
...
static esp_err_t step_right_handler(httpd_req_t *req){
    WheelAct(LOW, HIGH, HIGH, LOW);
    delay(100);
    WheelAct(LOW, LOW, LOW, LOW);
...
void startCameraServer(){
    ...
    httpd_uri_t go_uri = {
        ...
        httpd_uri_t step_go_uri = {
        .uri       = "/step_go",
        .method    = HTTP_GET,
        .handler   = step_go_handler,
        .user_ctx  = NULL
    };
    ...
    httpd_uri_t right_uri = {
        ...
        httpd_uri_t step_left_uri = {
        .uri       = "/step_left",
        .method    = HTTP_GET,
        .handler   = step_left_handler,
        .user_ctx  = NULL
    };
    
    httpd_uri_t step_right_uri = {
        .uri       = "/step_right",
        .method    = HTTP_GET,
        .handler   = step_right_handler,
        .user_ctx  = NULL
    };
    ...
    httpd_uri_t bling_uri = {
        .uri       = "/bling",
        .method    = HTTP_GET,
        .handler   = bling_handler,
        .user_ctx  = NULL
    };
   ...
   httpd_uri_t stream_uri = {
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &bling_uri);
        httpd_register_uri_handler(camera_httpd, &step_left_uri);
        httpd_register_uri_handler(camera_httpd, &step_right_uri);
        httpd_register_uri_handler(camera_httpd, &step_go_uri);
        httpd_register_uri_handler(camera_httpd, &step_command_uri);
    ...
    if (httpd_start(&stream_httpd, &config) == ESP_OK) {
        Serial.printf("start stream uri");
